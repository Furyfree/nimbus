...
check: fmt-check vet test
    ...
    if command -v shellcheck >/dev/null 2>&1; then shellcheck install.sh bootstrap tools/release/*.sh tools/vm/maintenance/*.sh system/root/etc/kernel/install.d/90-nimbus-uki.install system/root/etc/grub.d/09_nimbus_previous_kernels; else echo 'shellcheck not installed: skipped'; fi
