...
def verify(root):
    ...
    run("gpg", "--batch", "--passphrase", "", "--quick-generate-key",
        "Nimbus fixture <test@example.invalid>", "rsa2048", "sign", "1d")
    key = next(line.split(":")[9] for line in
               run("gpg", "--with-colons", "--list-keys").stdout.splitlines()
    ...
    def build(name, version, repo, requires=""):
        ...
        run("rpmsign", "--define", "_openpgp_sign gpg", "--define",
            f"_openpgp_sign_id {key}", "--addsign", str(rpm))
    ...
    def observed(name, version, source):
        actual = run("dnf5", "repoquery", "--installed", name,
                     "--queryformat", "%{version}|%{from_repo}").stdout.strip()
    ...
    print("PASS mixed source install, stronger competing priority, cross-source dependency")
    ...
    print("PASS constrained upgrade, orphan installed package, disabled source unchanged")

    run("dnf5", "-y", "do", "--action=upgrade", "--from-repo=nimbus-b",
        "nimbus-source-probe")
    ...
    refresh()
    run("dnf5", "-y", "reinstall", "--from-repo=nimbus-b",
        "nimbus-source-probe")
