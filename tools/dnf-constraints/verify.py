...
def verify(root):
    ...
    contains(
        preview("install", "hyprland", "noctalia", "hyprland-devel"),
        "0:0.56.1-2.fc44",
        "0:5.8.1-1.fc44",
        "hyprland-devel",
    ...
    for version in [
        ...
        contains(
            preview("install", f"hyprland-{version}-1.fc44"),
            "Failed to resolve the transaction",
            "excluded",
        )
    for version in ["4.9.0", "5.9.0~rc1", "6.0.0"]:
        contains(
            preview("install", f"noctalia-{version}-1.fc44"),
            "Failed to resolve the transaction",
            "excluded",
        )
    contains(
        preview("install", "hyprland-plugin"),
        "Failed to resolve the transaction",
        "hyprland >= 0.57",
    )
    ...
    print(
        "PASS: families, prereleases, epoch, subpackages, dependency conflict, "
