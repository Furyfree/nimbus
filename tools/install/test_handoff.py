...
class HandoffTests(unittest.TestCase):
    ...
    def run_fixture(self, nested, mode, sig, terminal_interrupt=False):
        ...
        command += (
            'bash "$INNER"'
            if nested
            else '"$PYTHON" -I -B "$WORKER" "$FIXTURE" "$MODE"'
        )
        reaped = False
        fd = None
        process = None
        ...
        try:
            ...
            self.assertGreaterEqual(
                finished[0].stat().st_mtime_ns,
                (root / "grandchild-stopped").stat().st_mtime_ns,
            )
            self.assertFalse(
                list((root / "state/nimbus/install").glob("run-*/.active"))
            )
            self.assertEqual(
                (root / "input").read_text(), "profile-from-" + mode + "\n"
            )
        finally:
            if not reaped:
    ...
    def test_launch_failure(self):
        result = subprocess.run(
            self.command("/missing-nimbus-test-command"),
            capture_output=True,
            text=True,
            start_new_session=True,
        )
...
"""
    ...
    def run_interactive_suspend(self, nested, initial_background=False):
        ...
        outer = root / "outer.sh"
        command = (
            'bash "$INNER"' if nested else '"$PYTHON" -I -B "$WORKER" "$FIXTURE" tty'
        )
        outer.write_text(
            'printf "%s" "$BASHPID" > "$FIXTURE/outer-pid"\n'
        ...
        def wait_for_background_stop():
            ...
            self.assertEqual(os.tcgetpgrp(fd), pid)
            self.assertFalse(
                list((root / "state/nimbus/install").glob("run-*/.finished"))
            )
...
"""
    ...
    def test_permission_denied_waits_for_descendants(self):
        ...
        worker = (
            "import os,time,pathlib; p=os.fork(); time.sleep(.25 if p==0 else 0); "
