...
with tempfile.TemporaryDirectory(prefix="nimbus-maintenance-") as directory:
    ...
    for version, name in (("0.4.6", "nimbus-old"), ("0.4.7", "nimbus-new")):
        subprocess.run(["go", "build", "-ldflags=" +
                        "-X github.com/Furyfree/nimbus/internal/version.Engine=" + version,
                        "-o", str(work / name), "./cmd/nimbus"],
    ...
    subprocess.run(["docker", "run", "--rm", "--network=none", "--platform=linux/amd64",
                    "-v", str(work) + ":/input:ro,Z", "--entrypoint", "/bin/bash",
                    image, "/input/test.sh"], check=True)
