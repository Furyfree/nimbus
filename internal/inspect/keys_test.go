...
func TestKeyFingerprintsDistinguishPrimaryKeysFromSubkeys(t *testing.T) {
	...
	src := &nativetest.FakeSource{Commands: map[string][]byte{nativetest.Key("gpg", KeyInspectArgs(path)...): out}}
	got, err := KeyFingerprints(src, path)
	if err != nil || !slices.Equal(got, []string{keyA}) {
		t.Fatalf("primary with subkey = %v, %v", got, err)
	}
	src.Commands[nativetest.Key("gpg", KeyInspectArgs(path)...)] = keyOutput(keyA, keyB)
	got, err = KeyFingerprints(src, path)
...
func TestRepeatedPrimaryKeysDoNotHideOtherSigners(t *testing.T) {
	path := filepath.Join(t.TempDir(), "public.asc")
	src := &nativetest.FakeSource{Commands: map[string][]byte{nativetest.Key("gpg", KeyInspectArgs(path)...): keyOutput(keyB, keyA, keyB, keyA)}}
	got, err := KeyFingerprints(src, path)
	if err != nil || !slices.Equal(got, []string{keyA, keyB}) {
		t.Fatalf("repeated primary keys = %v, %v", got, err)
	}
	src.Commands[nativetest.Key("gpg", KeyInspectArgs(path)...)] = keyOutput(keyA, keyA, "invalid")
...
func TestRepositoryKeysDeduplicateFilesWithoutLosingSigners(t *testing.T) {
	...
	src := &nativetest.FakeSource{Commands: map[string][]byte{
		nativetest.Key("gpg", KeyInspectArgs(first)...):  keyOutput(keyA, keyA),
		nativetest.Key("gpg", KeyInspectArgs(second)...): keyOutput(keyB, keyA),
	}}
	...
	delete(src.Commands, nativetest.Key("gpg", KeyInspectArgs(second)...))
...
func TestRepositoryKeyObservationUsesTheEffectiveOverride(t *testing.T) {
	src := &nativetest.FakeSource{
		...
		Commands: map[string][]byte{nativetest.Key("gpg", KeyInspectArgs("/etc/pki/rpm-gpg/pinned")...): keyOutput(keyA)},
	}
		...
		t.Run(fmt.Sprintf("override=%v", override), func(t *testing.T) {
			...
			src := &nativetest.FakeSource{
				...
				Commands: map[string][]byte{
					nativetest.Key("gpg", KeyInspectArgs("/etc/pki/rpm-gpg/pinned")...): keyOutput(keyA),
					nativetest.Key("gpg", KeyInspectArgs("/etc/pki/rpm-gpg/extra")...):  keyOutput(keyB),
				},
			}
			if override {
			...
			delete(src.Commands, nativetest.Key("gpg", KeyInspectArgs("/etc/pki/rpm-gpg/extra")...))
			repos, err = repositories(src)
...
func TestFlatpakTrustObservesInstalledKeyAndSignaturePolicy(t *testing.T) {
	src := &nativetest.FakeSource{
		Files:    map[string][]byte{filepath.Join(FlatpakRepoPath, "config"): []byte("[remote \"flathub\"]\ngpg-verify=false\n")},
		Commands: map[string][]byte{nativetest.Key("gpg", KeyInspectArgs(filepath.Join(FlatpakRepoPath, "flathub.trustedkeys.gpg"))...): keyOutput(keyA, keyB)},
	}
	remote := FlatpakRemote{Name: "flathub"}
	inspectRemoteTrust(src, &remote)
	...
	delete(src.Commands, nativetest.Key("gpg", KeyInspectArgs(filepath.Join(FlatpakRepoPath, "flathub.trustedkeys.gpg"))...))
	remote = FlatpakRemote{Name: "flathub"}
	inspectRemoteTrust(src, &remote)
	if remote.KeyError == "" {
