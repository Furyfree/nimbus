...
func TestUnknownFactsAreReportedNotGuessed(t *testing.T) {
	...
	src.Failures[nativetest.Key("dnf5", PackageQueryArgs...)] = "dnf5: cannot open the package database"
	src.Failures[nativetest.Key("systemctl", "is-active", "firewalld")] = "inactive"
	delete(src.Commands, nativetest.Key("systemctl", "is-active", "firewalld"))
	delete(src.Paths, "flatpak")
	delete(src.Commands, nativetest.Key("flatpak", "remotes", "--system", "--columns=name,url"))
	f := Inspect(src, "")
	...
	if f.Flatpak.Known() || !strings.Contains(f.Flatpak.Error, nativetest.ErrNotRecorded.Error()) {
		t.Fatalf("flatpak = %+v", f.Flatpak)
	}
...
func TestFirewalldStateFromFailingExit(t *testing.T) {
	...
	src.Commands[nativetest.Key("systemctl", "is-active", "firewalld")] = []byte("inactive\n")
	delete(src.Failures, nativetest.Key("systemctl", "is-active", "firewalld"))
	f := Inspect(src, "")
...
func TestPlatformReadsPastLongComments(t *testing.T) {
	src := &nativetest.FakeSource{
		...
		Commands: map[string][]byte{nativetest.Key("uname", "-m"): []byte("x86_64\n")},
	}
...
func TestFlatpakTrustReadsPastLongComments(t *testing.T) {
	src := &nativetest.FakeSource{
		...
		Commands: map[string][]byte{nativetest.Key("gpg", KeyInspectArgs(filepath.Join(FlatpakRepoPath, "flathub.trustedkeys.gpg"))...): keyOutput(keyA)},
	}
...
func TestFakeFailureKeepsRecordedOutput(t *testing.T) {
	src := fedora44(t)
	src.Commands[nativetest.Key("systemctl", "is-active", "firewalld")] = []byte("inactive\n")
	src.Failures[nativetest.Key("systemctl", "is-active", "firewalld")] = "exit status 3"
	f := Inspect(src, "")
...
func TestCheckoutReadsOnlyThroughSource(t *testing.T) {
	...
	for _, key := range []string{nativetest.Key("git", GitArgs(root, "rev-parse", "HEAD")...), nativetest.Key("git", GitArgs(root, "status", "--porcelain")...)} {
...
func TestCheckoutFollowsWorktreePointer(t *testing.T) {
	...
	src.Commands[nativetest.Key("git", GitArgs(wt, "rev-parse", "HEAD")...)] = []byte("abc\n")
	src.Commands[nativetest.Key("git", GitArgs(wt, "status", "--porcelain")...)] = []byte("")
	f := Inspect(src, wt)
	if !f.Checkout.Known() || f.Checkout.Value.Origin != "github.com/furyfree-org/nimbus" {
...
func TestChezmoiDataIsReadOnlyWhenInitialized(t *testing.T) {
	...
	src.Commands[nativetest.Key("chezmoi", ChezmoiDataArgs...)] = []byte(`{"Machine":"laptop","ManagedByNimbus":true,"Profiles":["common","development"],"profiles":["common","unix","linux","development"],"onePasswordSsh":false,"chezmoi":{"os":"linux"}}`)
	c, err := chezmoi(src)
...
func TestOptionalUserDirectoryErrorsRemainUnknown(t *testing.T) {
	...
	for _, cause := range []error{os.ErrPermission, syscall.EIO} {
		t.Run(cause.Error(), func(t *testing.T) {
			base := &nativetest.FakeSource{Paths: map[string]string{"chezmoi": "/usr/bin/chezmoi"}}
