...
func TestRepositoryLocationOverridesAreObserved(t *testing.T) {
	src := &nativetest.FakeSource{Dirs: map[string][]string{RepoDir: {"test.repo"}, RepoOverride: {"override.repo"}}, Files: map[string][]byte{
	...
	for _, comment := range []string{"# local override", "; local override"} {
		t.Run(comment, func(t *testing.T) {
			src := &nativetest.FakeSource{
				...
				Commands: map[string][]byte{nativetest.Key("gpg", KeyInspectArgs("/etc/pki/rpm-gpg/other")...): keyOutput(keyB)},
			}
			repos, err := repositories(src)
