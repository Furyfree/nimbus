...
func TestHardwareProposesComponentsAndMatchesAMachine(t *testing.T) {
	c := repositoryOnly(t)
	...
	if got := MatchMachine(c.Machines, laptop); got != "laptop" {
		t.Fatalf("laptop match = %q", got)
	}
	...
	if got := MatchMachine(c.Machines, desktop); got != "desktop" {
		t.Fatalf("desktop match = %q", got)
	}
	...
	if got := MatchMachine(c.Machines, vm); got != "" {
		t.Fatalf("vm must match no tracked machine, got %q", got)
	}
}
