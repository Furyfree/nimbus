...
var taskDescriptions = []struct{ id, title, help string }{
	...
	{"fde", "Set up TPM automatic disk unlock", "Requires the selected fde component and its recorded packages. Status inspects read-only: mounted LUKS2 root, TPM2 device, firmware mode, hook payload, marker, firmware entry and tools. After approval, the setup writes the marker, builds and signs /boot/efi/EFI/Linux/nimbus.efi with ukify and ensures the Nimbus UKI firmware entry; kernel updates rebuild the image through the engine-provided hook. Secure Boot enabled without an akmods signing key pair blocks; with Secure Boot disabled the image is unsigned. TPM enrollment, policy renewal and scoped removal are not implemented yet. Sync and upgrades never build images, enroll or change policy."},
