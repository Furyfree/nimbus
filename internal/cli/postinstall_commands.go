...
var taskDescriptions = []struct{ id, title, help string }{
	...
	{"fde", "Set up TPM automatic disk unlock", "Requires the selected fde component and its recorded packages. The task currently inspects read-only: mounted LUKS2 root, TPM2 device, firmware mode and tools; shim presence, EFI space and keyslots need the approved privileged check. Setup, enrollment and scoped removal are not implemented yet. Sync and upgrades never enroll a machine."},
