...
func TestCandidateDefinitionsRejectPrePhase6Release(t *testing.T) {
	saved := version.Engine
	t.Cleanup(func() { version.Engine = saved })
	for _, v := range []string{"0.1.1", "0.2.0", "0.0.0-dev.candidate"} {
		version.Engine = v
		code, out, errOut := run(t, "validate", "--checkout", repoRoot(t))
		if v == "0.1.1" {
			if code != ExitFailure || !strings.Contains(out+errOut, "min_engine 0.2.0 is newer") {
