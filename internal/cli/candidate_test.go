...
func TestCandidateDefinitionsRequireCompatibleEngine(t *testing.T) {
	...
	current := map[string]bool{"0.6.3": true, "0.6.1~dev.20260920193406": true, "0.0.0-dev.candidate": true}
	for _, v := range []string{"0.1.1", "0.2.0", "0.2.3", "0.3.0", "0.3.1", "0.3.2", "0.4.0", "0.4.1", "0.4.2", "0.4.3", "0.4.6", "0.5.0", "0.5.2", "0.5.3", "0.5.10", "0.6.0", "0.6.1", "0.6.2", "0.6.3", "0.6.1~dev.20260920193406", "0.0.0-dev.candidate"} {
		version.Engine = v
		code, out, errOut := run(t, "validate", "--checkout", repoRoot(t))
		if !current[v] {
			if code != ExitFailure || !strings.Contains(out+errOut, "min_engine 0.6.3 is newer") {
