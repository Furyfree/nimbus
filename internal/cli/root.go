...
func newRoot() (*cobra.Command, *options) {
	...
	root.AddCommand(newChannel(), newInternal(), newComponents(opts), newDoctor(opts), newFiles(opts), newInit(opts), newLaunch(opts), newManaged(opts), newPackages(opts), newPostinstall(opts), newProfiles(opts), newStatus(opts), newSetupNotes(opts), newSync(opts), newUnmanaged(opts), newUpgrade(opts), newValidate(opts), newVersion(opts), newWhy(opts))
	return root, opts
}
