...
func TestFinalReportRetainsFailuresAndNotices(t *testing.T) {
	...
	for _, text := range []string{"download failed", "Noctalia overrides", "Reboot required", "Run before rebooting: nimbus postinstall nvidia-mok (Sign NVIDIA modules and enroll their key)", "2 tasks, 1 setup note. Run: nimbus setup-notes", "Verification problems:", "Fingerprint device and enrollment state could not be established."} {
		if !strings.Contains(out.String(), text) {
			t.Fatal(out.String())
		}
	}
	if strings.Contains(out.String(), "Guidance") || strings.Contains(out.String(), "active Hyprland desktop session") || strings.Count(out.String(), "Run before rebooting") != 1 {
	...
	for _, text := range []string{"Logs: /tmp/logs", "Before rebooting:", "Sign NVIDIA modules and enroll their key: $ nimbus postinstall nvidia-mok", "Reboot to finish, then open a terminal and run:", "Remaining setup and guidance (2 tasks, 1 setup note): $ nimbus setup-notes"} {
		if !strings.Contains(finish.String(), text) {
			t.Fatal(finish.String())
		}
	}
	if strings.Contains(finish.String(), "kmodgenca") || strings.Contains(finish.String(), "Fingerprint") {
		t.Fatal("finish block carried guidance or verification problems", finish.String())
	}
	logout := r
