...
func fedora44(t *testing.T) *FakeSource {
	t.Helper()
	...
	src := &FakeSource{
		Commands: map[string][]byte{
			Key("uname", "-m"):                                          []byte("x86_64\n"),
			Key("dnf5", PackageQueryArgs...):                            read("repoquery-installed.txt"),
			Key("flatpak", "remotes", "--system", "--columns=name,url"): []byte("flathub\thttps://dl.flathub.org/repo/\n"),
			Key("flatpak", "list", "--system", "--app", "--columns=application,version,origin"): []byte("com.spotify.Client\t1.2.60\tflathub\nmd.obsidian.Obsidian\t1.8.10\tflathub\n"),
			Key("systemctl", "is-active", "firewalld"):                                          []byte("active\n"),
		},
