...
spec = importlib.util.spec_from_file_location("dtu_network", Path(__file__).with_name("network.py"))
...
class Credentials(unittest.TestCase):
    ...
    def test_private_op_fields_and_validation(self):
        good = [{"label": "username", "value": "s123456"},
                {"label": "password", "value": SECRET}]
        for fields in (good, good[:1], good + good[:1], {}, None,
                       [{"label": "username", "value": "x@other.org"}, good[1]],
                       [good[0], {"label": "password", "value": ""}]):
            result = subprocess.CompletedProcess([], 0, json.dumps(fields).encode(), b"")
            with self.subTest(fields=type(fields)), patch.object(dtu.subprocess, "run", return_value=result) as run:
                if fields == good:
    ...
    def test_op_failure_and_timeout_do_not_expose_native_output(self):
        ...
        for options in ({"return_value": result},
                        {"side_effect": subprocess.TimeoutExpired([SECRET], 1)},
                        {"side_effect": FileNotFoundError(SECRET)}):
            with patch.object(dtu.subprocess, "run", **options), self.assertRaises(dtu.SetupError) as error:
                dtu.onepassword(ITEM)
            self.assertNotIn(SECRET, str(error.exception))

    def test_manual_and_cancellation(self):
        with patch.object(dtu.sys.stdin, "isatty", return_value=True), \
        ...
        with patch.object(dtu.sys.stdin, "isatty", return_value=False), \
...
class Profiles(unittest.TestCase):
    def setUp(self):
        ...
        self.enterContext(patch.object(dtu, "CA_PATH", str(Path(__file__).with_name("ca.pem"))))
        self.creds = self.enterContext(patch.object(dtu, "credentials", return_value=("s123456@dtu.dk", SECRET)))
    ...
    def test_existing_profiles_require_approval_before_credentials_or_writes(self):
        for ssid in (b"eduroam", b"DTUsecure"):
            self.network = fixture()
            legacy = {"connection": {"uuid": "legacy", "id": "custom name", "type": "802-11-wireless"},
                      "802-11-wireless": {"ssid": ssid}}
            self.network.settings.AddConnection(legacy)
            self.assertEqual(self.network.status()["existing"], [{"uuid": "legacy", "ssid": ssid.decode()}])
            with self.assertRaises(dtu.SetupError):
                self.configure()
            self.creds.assert_not_called()
            self.assertEqual(self.network.settings.writes, 1)
            self.assertEqual(next(iter(self.network.settings.connections.values())).saved, legacy)

    def test_approved_replacement_preserves_unrelated_profiles(self):
        home = {"connection": {"uuid": "home", "type": "802-11-wireless"}, "802-11-wireless": {"ssid": b"home"}}
        self.network.settings.AddConnection(home)
        for ssid in (b"eduroam", b"DTUsecure"):
            self.network.settings.AddConnection({"connection": {"uuid": ssid.decode(), "type": "802-11-wireless"}, "802-11-wireless": {"ssid": ssid}})
        ...
        owned = next(c for c in self.network.settings.connections.values() if c.saved["connection"]["uuid"] == self.network.uuid)
    ...
    def test_failed_recreation_reports_partial_deletion_and_can_retry(self):
        self.configure()
        with patch.object(self.network.settings, "AddConnection", side_effect=RuntimeError(SECRET)):
            with self.assertRaisesRegex(dtu.SetupError, "after deleting") as error:
                self.configure(replace=True)
        self.assertNotIn(SECRET, str(error.exception))
    ...
    def test_inspection_never_fetches_secrets(self):
        ...
        self.creds.reset_mock()
        writes = self.network.settings.writes
        with patch.object(Connection, "GetSecrets", side_effect=AssertionError("secret inspection")):
            self.network.status()
    ...
    def test_manual_and_onepassword_credentials_are_saved_and_verified(self):
        ...
        for method in ("manual", "onepassword"):
            self.network = fixture()
            fields = [{"label": "username", "value": "s123456"},
                      {"label": "password", "value": SECRET}]
            result = subprocess.CompletedProcess([], 0, json.dumps(fields).encode(), b"")
            with patch.object(dtu, "credentials", REAL_CREDENTIALS), \
                 patch.object(dtu.sys.stdin, "isatty", return_value=True), \
                 patch("builtins.input", side_effect=["m", "s123456", "n"] if method == "manual" else ["n"]), \
                 patch.object(dtu.getpass, "getpass", return_value=SECRET), \
                 patch.object(dtu.subprocess, "run", return_value=result) as run:
                self.network.configure(self.network.observe()[1], "" if method == "manual" else ITEM)
    ...
    def test_missing_or_unreadable_saved_password_stops_before_connection(self):
        for options in ({"return_value": {}},
            ...
            self.network = fixture()
            with patch.object(Connection, "GetSecrets", **options), \
    ...
    def test_unknown_ownership_blocks_before_credentials(self):
        for meta in ({"uuid": "foreign", "id": dtu.PROFILE_ID},
    ...
    def test_drift_and_canceled_authorization_preserve_existing_state(self):
        ...
        def drift(_):
            self.network.settings.AddConnection(self.network.desired("other@dtu.dk", SECRET))
    ...
    def test_security_drift_needs_approved_recreation(self):
        self.configure()
        for key, value in (("domain-match", ""), ("ca-cert", b""),
                           ("system-ca-certs", True), ("phase1-auth-flags", 1),
                           ("password-flags", 1), ("identity", "foreign@example.org")):
            connection = next(iter(self.network.settings.connections.values()))
    ...
    def test_connect_requires_confirmation_but_autoconnect_does_not(self):
        ...
        for success in (False, True):
            active = [False]
            self.network.active = lambda: active[0]
    ...
    def test_connection_timeout_retains_profile_and_autoconnect(self):
        with patch.object(dtu.sys.stdin, "isatty", return_value=True), \
             patch("builtins.input", return_value="y"), \
             patch.object(self.network, "eduroam_visible", return_value=True), \
             patch.object(dtu.subprocess, "run", side_effect=subprocess.TimeoutExpired([SECRET], 75)), \
    ...
    def test_off_campus_setup_succeeds_without_attempting_activation(self):
        for terminal in (False, True):
            ...
            with patch.object(dtu.sys.stdin, "isatty", return_value=terminal), \
                 patch("builtins.input", return_value="y"), \
                 patch.object(dtu.subprocess, "run", return_value=subprocess.CompletedProcess([], 0, b"Home\n", b"")) as run:
                self.configure()
            connection = next(iter(self.network.settings.connections.values()))
            self.assertTrue(connection.saved["connection"]["autoconnect"])
            self.assertEqual(connection.secret, SECRET)
    ...
    def test_scan_matches_exact_ssid_and_keeps_native_output_private(self):
        for output, expected in ((b"eduroam\n", True), (b"Home\neduroam\n", True),
                                 (b"", False), (b"eduroam-guest\n", False)):
            with patch.object(dtu.subprocess, "run", return_value=subprocess.CompletedProcess([], 0, output, b"")):
                self.assertEqual(self.network.eduroam_visible(), expected)
        for options in ({"return_value": subprocess.CompletedProcess([], 1, b"", SECRET.encode())},
    ...
    def test_success_accepts_native_omission_of_default_autoconnect(self):
        ...
        with patch.object(Connection, "Update", native_update), \
             patch.object(dtu.sys.stdin, "isatty", return_value=True), \
             patch("builtins.input", return_value="y"), \
             patch.object(dtu.subprocess, "run", return_value=subprocess.CompletedProcess([], 0)):
            self.configure()
