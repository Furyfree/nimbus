...
def onepassword(item):
    if not re.fullmatch(r"[a-z0-9]{26}", item):
        raise SetupError("Supply the 26-character 1Password item UUID, not a vault UUID or item title.")
    try:
        ...
        raise SetupError("Install 1Password CLI and enable desktop CLI integration, or choose manual entry.") from None
    except subprocess.TimeoutExpired:
        raise SetupError("1Password authorization timed out; unlock 1Password and retry.") from None
    if result.returncode:
        raise SetupError("1Password could not read the item; check authorization, account and item UUID.")
    try:
        ...
        raise SetupError("The 1Password item must have nonempty username and password fields.") from None
...
def credentials(item):
    ...
    if not sys.stdin.isatty():
        raise SetupError("Manual credential selection requires a terminal; use --onepassword-item for 1Password.")
...
class Network:
    def __init__(self):
        ...
        self.settings = self.interface("/org/freedesktop/NetworkManager/Settings",
                                       SERVICE + ".Settings")
    ...
    def observe(self):
        ...
        for path in self.settings.ListConnections():
            ...
            if meta.get("type") == "802-11-wireless" and ssid in (b"eduroam", b"DTUsecure"):
                if not meta.get("uuid"):
                    raise SetupError("A matching Wi-Fi profile has no UUID; review it manually.")
            ...
            if meta.get("uuid") != self.uuid:
                if meta.get("id") == PROFILE_ID:
                    raise SetupError("Another connection uses the Nimbus DTU name; review it manually before setup.")
                continue
            if found is not None:
                raise SetupError("Multiple Nimbus DTU profiles found; review them manually.")
            permissions = list(meta.get("permissions", []))
            marker = settings.get("user", {}).get("data", {}).get("nimbus.dtu.owner")
            if (meta.get("id") != PROFILE_ID or marker != self.user
                    or permissions != ["user:" + self.user + ":"]):
                raise SetupError("The expected DTU profile has unfamiliar ownership; it will not be overwritten.")
    ...
    def valid(self, settings):
        ...
        try:
            correct_identity = identity(str(eap.get("identity", ""))) == eap.get("identity")
        except SetupError:
            correct_identity = False
        return (settings.get("connection", {}).get("type") == "802-11-wireless"
                and wifi.get("mode") == "infrastructure"
                and correct_identity and bytes(wifi.get("ssid", [])) == b"eduroam"
    ...
    def active(self):
        props = self.interface("/org/freedesktop/NetworkManager",
    ...
    def status(self):
        ...
        return {"observed": observed, "configured": bool(found and self.valid(found[1])),
    ...
    def desired(self, username, password):
        ...
        return dbus.Dictionary({
            "connection": dbus.Dictionary({
                "type": "802-11-wireless", "uuid": self.uuid, "id": PROFILE_ID,
                "permissions": dbus.Array(["user:" + self.user + ":"], signature="s"),
                "autoconnect": dbus.Boolean(False),
            }, signature="sv"),
            "user": dbus.Dictionary({"data": dbus.Dictionary(
                {"nimbus.dtu.owner": self.user}, signature="ss")}, signature="sv"),
            "802-11-wireless": dbus.Dictionary({
                "ssid": dbus.ByteArray(b"eduroam"), "mode": "infrastructure",
                "security": "802-11-wireless-security",
            }, signature="sv"),
            "802-11-wireless-security": dbus.Dictionary({
                "key-mgmt": "wpa-eap", "proto": dbus.Array(["rsn"], signature="s"),
                "pairwise": dbus.Array(["ccmp"], signature="s"),
                "group": dbus.Array(["ccmp"], signature="s"),
            }, signature="sv"),
            "802-1x": dbus.Dictionary({
                "eap": dbus.Array(["peap"], signature="s"), "identity": username,
                "ca-cert": dbus.ByteArray(("file://" + CA_PATH + "\0").encode()),
                "domain-match": SERVERS, "phase2-auth": "mschapv2",
                "anonymous-identity": "anonymous@dtu.dk", "password": password,
    ...
    def configure(self, expected, item, replace_existing=False):
        ...
        if observed != expected:
            raise SetupError("DTU connection changed after approval; inspect and retry.")
        if self.existing and not replace_existing:
            raise SetupError("Existing eduroam/DTUsecure profiles were kept; explicit replacement approval is required.")
        with open(CA_PATH, "rb") as f:
            if hashlib.sha256(f.read(65537)).hexdigest() != CA_SHA256:
                raise SetupError("The reviewed DTU CA bundle is missing or changed; rerun certificate preparation.")
        ...
        if observed != expected:
            raise SetupError("DTU connection changed during authorization; inspect and retry.")
        ...
        try:
            ...
            for path, uid, _ in targets:
                ...
                if self.snapshots != approved:
                    raise SetupError("DTU profiles changed during replacement; inspect and retry.")
                self.interface(path, CONNECTION).Delete()
                removed = True
                approved.pop(uid)
                self.observe()
                if any(current_uid == uid for _, current_uid, _ in self.existing):
                    raise SetupError("Profile deletion could not be verified; inspect and retry.")
            if self.existing:
                raise SetupError("Another DTU profile appeared; setup stopped before creating a duplicate.")
            path = self.settings.AddConnection(settings)
        except Exception:
            if removed:
                raise SetupError("Replacement stopped after deleting an approved profile. Old credentials cannot be restored by Nimbus; rerun setup or restore it through NetworkManager.") from None
            raise SetupError("NetworkManager could not replace/create the profile; inspect its permissions and retry.") from None
        else:
            self.verify_password(path, password)
        finally:
            del password, settings
        if not self.status()["configured"]:
            raise SetupError("Profile was written but configuration verification failed; inspect and retry.")
        ...
        if not current or not self.valid(current[1]):
            raise SetupError("Connection settings changed; automatic connection was not enabled.")
        settings = current[1]
        ...
        if not current or not current[1]["connection"].get("autoconnect", True):
            raise SetupError("Profile saved, but automatic connection could not be verified; check the network menu.")
        print("DTU eduroam profile and saved password verified; automatic connection enabled.")
        print("NetworkManager owns password storage in its root-only connection file.")
        print("Only explicitly approved eduroam/DTUsecure profiles were replaced; other Wi-Fi profiles were preserved.")
        if not sys.stdin.isatty():
        ...
        if choice not in ("y", "yes"):
            ...
            return
        if self.active():
            print("Already connected to DTU eduroam; automatic connection enabled. Check Internet access and DTU services.")
            return
        if not self.eduroam_visible():
            print("eduroam was not found nearby. Setup is complete; your profile and password are saved.")
            self.connection_deferred()
            return
        # Keep native errors private: DBus/nmcli errors can contain account data.
        try:
            result = subprocess.run(["nmcli", "--wait", "60", "connection", "up", "uuid", self.uuid],
                                    stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                    timeout=75, check=False)
        except subprocess.TimeoutExpired:
            raise SetupError("Profile and password saved, but connection timed out. Retry this profile in the network menu; no recreation is needed.") from None
        if result.returncode or not self.active():
            raise SetupError("Profile and password saved, but connection failed. Check Wi-Fi range and DTU credentials; retry this profile in the network menu. No recreation is needed.")
        print("Connected to DTU eduroam; automatic connection enabled. Check Internet access and DTU services.")

    @staticmethod
    def connection_deferred():
        print("NetworkManager will try to connect automatically when eduroam is available and Wi-Fi is enabled. No need to rerun setup.")
        print("Connection has not been tested by this setup run; verify it when you are in range.")
    ...
    def eduroam_visible():
        ...
        try:
            result = subprocess.run(
                ["nmcli", "--colors", "no", "--terse", "--fields", "SSID",
                 "device", "wifi", "list", "--rescan", "yes"],
                stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=25,
                check=False,
            )
        except (OSError, subprocess.TimeoutExpired):
            raise SetupError("Profile saved, but available Wi-Fi networks could not be checked. Check Wi-Fi and NetworkManager, then retry in the network menu.") from None
        if result.returncode:
            raise SetupError("Profile saved, but available Wi-Fi networks could not be checked. Check Wi-Fi and NetworkManager, then retry in the network menu.")
    ...
    def verify_password(self, path, expected):
        ...
        try:
            ...
            raise SetupError("Profile created, but saved password verification failed. Inspect NetworkManager permissions and rerun setup.") from None
        if not matched:
            raise SetupError("Profile created, but NetworkManager did not retain the supplied password. Rerun setup; connection was not attempted.")
...
def main():
    ...
    parser.add_argument("--replace-existing", action="store_true", help="internal: CLI explicitly approved the observed replacement set")
    parser.add_argument("--onepassword-item", default="")
    args = parser.parse_args()
    if sys.platform != "linux" or os.getuid() == 0:
        raise SetupError("DTU setup requires Linux and your normal desktop user, not root.")
    network = Network()
...
if __name__ == "__main__":
    try:
        ...
        print("DTU setup failed. Check NetworkManager permissions and Python DBus support, then retry.", file=sys.stderr)
        sys.exit(1)
