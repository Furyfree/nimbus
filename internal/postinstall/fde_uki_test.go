...
func TestBuildFDEUKI(t *testing.T) {
	t.Run("installs atomically", func(t *testing.T) {
		...
		if err := buildFDEUKI(src, fdeTestVersion, io.Discard, target); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("a failed signature check keeps the previous image", func(t *testing.T) {
		...
		if err := buildFDEUKI(src, fdeTestVersion, io.Discard, target); err == nil || !strings.Contains(err.Error(), "did not verify") {
			t.Fatalf("got %v", err)
		}
	...
	t.Run("the key material is required and never generated by the hook", func(t *testing.T) {
		...
		err := buildFDEUKI(src, fdeTestVersion, io.Discard, filepath.Join(t.TempDir(), "nimbus.efi"))
	...
	t.Run("an undersized image is rejected", func(t *testing.T) {
		...
		src.Commands[nativetest.Key(FDEUKITool, fdeBuildArgv(target, false)...)] = []byte{}
		err := buildFDEUKI(src, fdeTestVersion, io.Discard, target)
	...
	t.Run("gate and version are enforced", func(t *testing.T) {
		...
		if err := buildFDEUKI(src, fdeTestVersion, io.Discard, filepath.Join(t.TempDir(), "nimbus.efi")); err == nil || !strings.Contains(err.Error(), "marker is absent") {
			t.Fatalf("got %v", err)
		}
		src = fdeBuildSource(t)
		src.Files[FDEUKIMarker] = []byte("foreign")
		if err := buildFDEUKI(src, fdeTestVersion, io.Discard, filepath.Join(t.TempDir(), "nimbus.efi")); err == nil || !strings.Contains(err.Error(), "unfamiliar content") {
			t.Fatalf("got %v", err)
		}
	})
}
